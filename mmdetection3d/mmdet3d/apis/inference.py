...
def inference_multi_modality_detector_feature(model: nn.Module,
                                      pcds: Union[str, Sequence[str]],
                                      imgs: Union[str, Sequence[str]],
                                      ann_file: Union[str, Sequence[str]],
                                      cam_type: str = 'CAM2',
                                      camera_view_list: str = None,
    ...
    #####Generally at start
    #/home/imr555/Downloads/projects/mmdet3d_installation/mmdetection3d/demo/data/nuscenes/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin
    #/home/imr555/Downloads/projects/mmdet3d_installation/mmdetection3d/demo/data/nuscenes/
    #/home/imr555/Downloads/projects/mmdet3d_installation/mmdetection3d/demo/data/nuscenes/n015-2018-07-24-11-22-45+0800.pkl
    #all

    #pcds can be covered with lidar_path
    #imgs is image folder, we have to manipulate it for path with all values
    #ann_file, no pkl file so pull from NuScenes directly under section 4 https://github.com/nutonomy/nuscenes-devkit/blob/master/python-sdk/tutorials/nuscenes_tutorial.ipynb
    #cam_type, manipulate with imgs
    ...
    data_list_act = []
    print(len(data_list))
    for sample_dict in data_list:
        if sample_dict["token"] == sample_token:
            count +=1
            data_list_act.append(sample_dict)
    ...
    for index, pcd in enumerate(pcds):
        ...
        if cam_type != 'all':
            ...
            count_index=0
            for _, img_info in data_info['images'].items():
                img_info['img_path'] = camera_view_list[count_index]
                assert osp.isfile(img_info['img_path']
                                  ), f'{img_info["img_path"]} does not exist.'
                count_index += 1
    ...
    # forward the model
    with torch.no_grad():
        results, feats = model.test_step(collate_data)

    if not is_batch:
        return results[0], data[0], feats[0]
    else:
        return results, data, feats
