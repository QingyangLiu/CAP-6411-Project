...
# Evaluation
@torch.no_grad()
def test_engine(__C, dataset, state_dict=None, save_eval_result=False, pre_net=None):
    ...
    for step, (
        ...
        obj_B, obj_N, obj_C=obj_feat_iter.shape  
        bbox_B, bbox_N, bbox_C=bbox_feat_iter.shape  

        obj_feat_iter=torch.reshape(obj_feat_iter, (int(obj_B*obj_N/100), 100, obj_C))
        bbox_feat_iter=torch.reshape(bbox_feat_iter, (int(bbox_B*bbox_N/100), 100, bbox_C))
        ques_ix_iter_temp=ques_ix_iter.repeat_interleave(10, dim=0)
              
        __C.USE_BBOX_FEAT=True
        aggregate=pre_net(obj_feat_iter, bbox_feat_iter, ques_ix_iter_temp, return_norm_only=True)
