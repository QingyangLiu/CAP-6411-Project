...
# Evaluation
@torch.no_grad()
def test_engine(__C, dataset, state_dict=None, save_eval_result=False, backbone_model=None):
    ...
    for step, (
        ...
        if backbone_model!=None:
            B0, N0, E0=obj_feat_iter.shape
            obj_feat_iter=torch.reshape(obj_feat_iter, (B0, int(N0/100), 100, E0))
            
            B0, N0, E0=bbox_feat_iter.shape
            bbox_feat_iter=torch.reshape(bbox_feat_iter, (B0, int(N0/100), 100, E0))


            B, N, C, E=obj_feat_iter.shape
            obj_feat_iter=torch.reshape(obj_feat_iter, (B*N, C, E))

            B1, N1, C1, E1=bbox_feat_iter.shape
            bbox_feat_iter=torch.reshape(bbox_feat_iter, (B1*N1, C1, E1))
            obj_feat_iter=backbone_model(obj_feat_iter, bbox_feat_iter)
            obj_feat_iter=torch.reshape(obj_feat_iter, (B, N, E))
