...
class NuScenes_QA(Data.Dataset):
    def __init__(self, __C):
        ...
        tokens=[]
        for question in self.qa_list:
            tokens.append(question['sample_token'])
        
        tokens= list(dict.fromkeys(tokens))
        self.token_dict={}
        num_frames=11

        for x in range(len(tokens)-num_frames+1):
            self.token_dict[tokens[x]]={}
            self.token_dict[tokens[x]]['nearest']=[]
            for y in range(1, num_frames):
                self.token_dict[tokens[x]]['nearest'].append(tokens[x+y])

        for questions in self.qa_list:
            if questions['sample_token'] not in self.token_dict:
                continue
            if 'questions' in self.token_dict[questions['sample_token']]:
                self.token_dict[questions['sample_token']]['questions'].append((questions['question'], questions['answer']))
            else:
                self.token_dict[questions['sample_token']]['questions']=[]
                self.token_dict[questions['sample_token']]['questions'].append((questions['question'], questions['answer']))


        print(len(self.token_dict.keys()))
        print(len(tokens))
    ...
    def __getitem__(self, idx):
        ...
        obj_feat_iter=[]
        bbox_feat_iter=[]
        obj_feat, bbox_feat = self.load_obj_feats(scene_token)
        obj_feat_iter.append(obj_feat)
        bbox_feat_iter.append(bbox_feat)

        for token in self.token_dict[scene_token]['nearest']:
            obj_feat, bbox_feat=self.load_obj_feats(token)
            obj_feat_iter.append(obj_feat)
            bbox_feat_iter.append(bbox_feat)
        obj_feat_iter=np.array(obj_feat_iter)
        bbox_feat_iter=np.array(bbox_feat_iter)
    ...
    def __len__(self):
        return len(self.token_dict.keys())
    ...
    def load_ques_ans(self, idx):
        scene_token=list(self.token_dict.keys())[idx]
        #print(self.token_dict[idx]['questions'])
        ques=random.choice(self.token_dict[scene_token]['questions'])
        #print(ques)
        #input('continue')

        ques_ix_iter = self.proc_ques(ques[0], max_token=30)
        ans_iter = np.zeros(1)
