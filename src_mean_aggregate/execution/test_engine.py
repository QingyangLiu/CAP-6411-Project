...
def test_engine(__C, dataset, state_dict=None, save_eval_result=False):
    ...
    for step, (
        ...
        aggregate=net(obj_feat_iter, bbox_feat_iter, ques_ix_iter_temp)
                
        aggregate=torch.reshape(aggregate, (obj_B, 10, -1))
        #Written by Qingyang Liu: END

        pred = pred=torch.mean(aggregate, dim=1)
