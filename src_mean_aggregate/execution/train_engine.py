...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    logfile.close()
    #test_engine(__C, dataset_eval, state_dict=net.state_dict(), save_eval_result = False)
    # Training script
    print(start_epoch)
    print(__C.MAX_EPOCH)
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                #input('continue')e
                #__C.HIDDEN_SIZE=512
                aggregate=net(sub_obj_feat_iter, sub_bbox_feat_iter, sub_ques_ix_iter_temp)
                
                aggregate=torch.reshape(aggregate, (obj_B, 10, -1))
                pred=torch.mean(aggregate, dim=1)
